...
class CategoriasController {
  ...
  public function getCategoriasController(){
    ...
    public function getCategoriasController2(){
      $respuesta = CategoriasModel::getCategoriasModel2('categorias');
       foreach ($respuesta as $row) {
         echo '<tr> 
              <td align="center"> '. $row['nombrecategoria'].'</td>
              <td align="center">
               <a class="fa fa-recycle btn btn-primary  btn-sm" href="index.php?action=categorias_bk&idRestaurar='.$row['idcategoria'].'" &nbsp;  </a>
              </td>
              </tr>';
       }
    }
  ...
  public function deleteCategoriasController(){
   ...
   public function restaurarCategoriasController(){
      if (isset($_GET['idRestaurar'])) {
       $datosController = $_GET['idRestaurar'];
       echo "OK_OK";
    #pedir la informacion al modelo.
       $respuesta = CategoriasModel::restaurarCategoriasModel($datosController,'categorias');
       if ($respuesta == 'success') {
          header('location:categorias_bk');
       }
      }
    }
