...
define("BACKUP_DIR", '../../assets/backup'); // Comment this line to use same script's directory ('.')
...
$backupDatabase->obfPrint($result, 1);
...
class Restore_Database {
    ...
    public function restoreDb() {
        try {
            ...
            //$files1=scandir($backupDir,1);
            //$backupFile = $this->backupFile;
            
            $files1 = array();
                $dir = realpath($backupDir);
                $di = new DirectoryIterator($dir);
                foreach ($di as $fileinfo) {
                    if (!$fileinfo->isDot()) {
                        $files1[] = $fileinfo->getRealPath();
                    }
                }
